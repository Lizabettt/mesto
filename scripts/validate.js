...
// покажем ошибку
const showInputError = (formPopup, inputFormPopup, errorMessage, validationConfig) => {
...
const checkInputValidity = (formPopup, inputFormPopup, validationConfig) => {
  if (!inputFormPopup.validity.valid) { // валидны ли введенные данные в инпут 
    showInputError(formPopup, inputFormPopup, inputFormPopup.validationMessage, validationConfig);
    } else {
...
function setEventListeners(formPopup, validationConfig) {
  ...
  const inputList = Array.from(formPopup.querySelectorAll(validationConfig.inputFormPopup));
...
function enableValidation(validationConfig) {
  ...
  const formList = Array.from(document.querySelectorAll(validationConfig.formPopup));
